...
const UserInfo = () => {
      ...
      if (isUserSex === -1) {
        ...
        await AsyncStorage.setItem(
          ...
          JSON.stringify({
            ...
            weight: isUserWeight,
            value,
            dailyRef: _.isNil(isDailyRefAmount)
              ? (Math.ceil(isUserWeight * 30 * 0.01) * 0.1).toFixed(1) * 1000
              : isDailyRefAmount
          })
        );
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <Container>
        <SpaceBetweenTouch>
          <BoldText>유저 신체 정보</BoldText>
        </SpaceBetweenTouch>
        <SpaceBetweenView>
          <BoldText>성별</BoldText>
          <SelectContainer>
            <SelectBox
              onPress={() => isSelectSex(0)}
              style={{
                backgroundColor: isUserSex === 0 ? "#81b0ff" : "#767577",
                borderTopLeftRadius: "4px",
                borderBottomLeftRadius: "4px"
              }}
            >
              <BoldText
                style={{ color: isUserSex === 0 ? "#f5dd4b" : "#f4f3f4" }}
              >
                남성
              </BoldText>
            </SelectBox>
            <SelectBox
              onPress={() => isSelectSex(1)}
              style={{
                backgroundColor: isUserSex === 1 ? "#81b0ff" : "#767577",
                borderTopRightRadius: "4px",
                borderBottomRightRadius: "4px"
              }}
            >
              <BoldText
                style={{ color: isUserSex === 1 ? "#f5dd4b" : "#f4f3f4" }}
              >
                여성
              </BoldText>
            </SelectBox>
        ...
        <SpaceBetweenTouch>
          <BoldText>섭취 정보</BoldText>
        </SpaceBetweenTouch>
        <SpaceBetweenView>
          <BoldText>1회 섭취량</BoldText>
          <TouchableOpacity onPress={() => pickerRef.current.show()}>
            {_.isNil(value) ? (
              <BoldText style={{ color: "#81b0ff" }}>설정</BoldText>
            ) : (
              <BoldText>{value}ml</BoldText>
            )}
          </TouchableOpacity>
        </SpaceBetweenView>
        <View>
          <SpaceBetweenView>
            <BoldText>일일 기준량</BoldText>
            <SelectContainer>
              <TouchableOpacity onPress={() => pickerLiterRef.current.show()}>
                {_.isNil(isDailyRefAmount) ? (
                  <BoldText>
                    {(Math.ceil(isUserWeight * 30 * 0.01) * 0.1).toFixed(1)}L
                  </BoldText>
                ) : (
                  <BoldText>
                    {(Math.ceil(isDailyRefAmount) * 0.001).toFixed(1)}L
                  </BoldText>
                )}
              </TouchableOpacity>
            </SelectContainer>
          ...
          >
            <RegularText style={{ fontSize: 14, color: "#767577" }}>
              하루 적정 기준량은 (체중 * 30) 입니다.
            </RegularText>
          </View>
        </View>
      </Container>
      <View style={{ paddingVertical: 20, paddingHorizontal: 20 }}>
        <TouchableOpacity
          onPress={() => isSubmitUserInfo()}
          style={{
            display: "flex",
            justifyContent: "center",
            alignItems: "center",
            height: 50,
            backgroundColor: !isConfirmButton ? "#81b0ff" : "#767577",
            borderRadius: 8
          }}
        >
          <BoldText style={{ color: !isConfirmButton ? "#f5dd4b" : "#f4f3f4" }}>
            저장
          </BoldText>
        </TouchableOpacity>
      </View>
